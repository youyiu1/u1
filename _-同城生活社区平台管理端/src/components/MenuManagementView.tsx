...
export default function MenuManagementView() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('leju_system_menus');
    if (saved) {
      try {
      ...
      setMenus(DEFAULT_MENUS);
      localStorage.setItem('leju_system_menus', JSON.stringify(DEFAULT_MENUS));
    }
  }, []);
  ...
  const saveMenus = (updated: SystemMenu[]) => {
    ...
    setMenus(sorted);
    localStorage.setItem('leju_system_menus', JSON.stringify(sorted));
  };
