...
- **账号安全**：Spring Security + JWT + Redis 登录态、图形验证码、邮箱验证码、登录限流、重置密码限流。
...
| Spring Security + JWT + Redis | 无状态安全配置、Token 签发、登录态双校验和安全响应头 |
...
| 用户登录 | 图形验证码后端校验，Spring Security + JWT + Redis 登录态 |
