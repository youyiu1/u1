  ...
  return (
    ...
    >
      <div className="space-y-4">
        <ToggleRow settingsKey="pushEnabled" label="总通知开关" desc="关闭后不再接收站内提醒" settings={settings} onToggle={handleToggle} />
        <ToggleRow settingsKey="messageNotify" label="私信通知" desc="收到新私信时接收提醒" settings={settings} onToggle={handleToggle} disabled={!settings.pushEnabled} />
        <ToggleRow settingsKey="followNotify" label="关注通知" desc="有新粉丝时接收提醒" settings={settings} onToggle={handleToggle} disabled={!settings.pushEnabled} />
        <ToggleRow settingsKey="likeNotify" label="点赞通知" desc="动态被点赞时接收提醒" settings={settings} onToggle={handleToggle} disabled={!settings.pushEnabled} />
        <ToggleRow settingsKey="commentNotify" label="评论通知" desc="收到评论时接收提醒" settings={settings} onToggle={handleToggle} disabled={!settings.pushEnabled} />
        <ToggleRow settingsKey="systemNotify" label="系统通知" desc="接收系统公告和更新通知" settings={settings} onToggle={handleToggle} disabled={!settings.pushEnabled} />
      </div>
